...
{
    public class CharacterRandomAttack : ICharacterAttack
    {
        private DamageDealer _baseDamageDealer;
        private Character _target;

        public CharacterRandomAttack(DamageDealer baseDamageDealer)
        {
        ...
        private Character FindTarget(Vector2Int characterPosition, TeamField enemyField)
        {
            ...
            var possibleTargetsPosition = targetingPossibility.GetPossibleTargets(PossibleTarget.AnyPositionWithCharacter);
            ...
            if (field.PositionDictionary.TryGetValue(randPosition, out var targetPosition) && !targetPosition.IsEmpty())
            {
